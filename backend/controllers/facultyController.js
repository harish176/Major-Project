...
export const registerFaculty = async (req, res) => {
  try {
    ...
    if (facultyData.panNumber) {
      const existingPan = await Faculty.findOne({
        panNumber: facultyData.panNumber,
      });
      if (existingPan) {
        return res.status(400).json({
          success: false,
    ...
    if (facultyData.aadharNumber) {
      const existingAadhar = await Faculty.findOne({
    ...
    if (
      facultyData.previousInstitutions &&
      facultyData.previousInstitutions.length === 0
    ) {
    ...
    if (facultyData.areasOfExpertise) {
      facultyData.areasOfExpertise = facultyData.areasOfExpertise.filter(
        (item) => item && item.trim() !== "",
      );
    }
    if (facultyData.subjectsTaught) {
      facultyData.subjectsTaught = facultyData.subjectsTaught.filter(
        (item) => item && item.trim() !== "",
      );
    }
    if (facultyData.researchInterests) {
      facultyData.researchInterests = facultyData.researchInterests.filter(
        (item) => item && item.trim() !== "",
      );
    }
    ...
    if (error.name === "ValidationError") {
      ...
      return res.status(400).json({
        success: false,
        message:
          "Validation failed: " + Object.keys(validationErrors).join(", "),
  ...
  try {
    ...
    if (!["active", "inactive", "retired", "terminated"].includes(status)) {
      return res.status(400).json({
        success: false,
        message:
          "Invalid status. Must be active, inactive, retired, or terminated",
      });
    }
...
export const getAdministrationFaculty = async (req, res) => {
  try {
    ...
    // Treat missing isActive as active for older records that were created before this field existed.
    ...
    if (category) {
      ...
      const categoryRules = {
        chairperson: [
          ...
          { category: "director" },
          { designation: /chairperson|chairman/i },
          { designation: /director/i },
          { "administrativeRoles.role": /chairperson|chairman/i },
        ],
        director: [
          { category: "director" },
          { designation: /director/i },
          { "administrativeRoles.role": /director/i },
        ],
        dean: [
          { category: "dean" },
          { designation: /dean/i },
          { "administrativeRoles.role": /dean/i },
        ],
        registrar: [
          { category: "registrar" },
          { designation: /registrar/i },
          { "administrativeRoles.role": /registrar/i },
        ],
        "finance-committee": [
          { category: "finance-committee" },
          { designation: /finance|accounts?/i },
          { "administrativeRoles.role": /finance|accounts?/i },
        ],
        faculty: [
          { category: "faculty" },
          { designation: /faculty/i },
          { "administrativeRoles.role": /faculty/i },
        ],
      };

      const categoryConditions = categories.flatMap((cat) => {
        if (categoryRules[cat]) {
          return categoryRules[cat];
        }

        return [
          { category: cat },
          { designation: new RegExp(cat, "i") },
          { "administrativeRoles.role": new RegExp(cat, "i") },
        ];
      });
    ...
    const people = await Faculty.find(query)
      .select(
        "firstName middleName lastName email contactNumber department designation category branch bio photo profileImage subjectsTaught areasOfExpertise researchInterests committees administrativeRoles displayOrder",
      )
